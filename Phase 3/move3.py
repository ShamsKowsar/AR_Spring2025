...
class ParticleFilter:
    ...
    def predict(self, v, w, dt):
        for p in self.particles:
            ...
            p.position.x += v_noisy * np.cos(yaw) * dt
            p.position.y += v_noisy * np.sin(yaw) * dt
...
class LocalizationNode:
    def __init__(self):
        ...
        self.world_file_path = rospy.get_param('~world_file', "/home/sepehr/catkin_ws/src/anki_description/world/sample1.world")
        self.num_particles = rospy.get_param('~num_particles', 800)
        self.convergence_threshold = rospy.get_param('~convergence_threshold', 0.002)
        ...
        self.map_bounds = (min(all_x)-1, max(all_x)+1, min(all_y)-1, max(all_y)+1)
    ...
    def setup_plot(self):
        with self.plot_lock:
            ...
            for poly in self.wall_polygons:
                self.ax.fill(*poly.exterior.xy, alpha=0.5, fc='gray', ec='black')
            
            self.particles_plot, = self.ax.plot([], [], 'o', color='orange', markersize=2, alpha=0.5, label='Particles')
            self.actual_quiver = self.ax.quiver(0, 0, 0, 0, color='red', alpha=0.7, scale=1, scale_units='xy', angles='xy', label='Actual Pose')
            self.estimated_quiver = self.ax.quiver(0, 0, 0, 0, color='blue', alpha=0.7, scale=1, scale_units='xy', angles='xy', label='Estimated Pose')
    ...
    def visualize(self):
        with self.plot_lock:
            self.particles_plot.set_data([p.position.x for p in self.particle_filter.particles],
            ...
            def update_quiver(quiver, pose):
                if pose:
                    ...
                    quiver.set_offsets([None, None])
                    quiver.set_UVC(None, None)
