...
class ParticleFilter:
    def __init__(self, num_particles, map_polygons, map_bounds):
        ...
        self.max_entropy = np.log2(self.num_particles) if self.num_particles > 0 else 0
        
        
        
        self.entropy_threshold_ratio = 0.25  
        self.entropy_threshold = self.max_entropy * self.entropy_threshold_ratio

        
        self.min_injection_ratio_entropy = 0.05
        self.max_injection_ratio_entropy = 0.5
    ...
    def calculate_entropy(self):
        ...
        epsilon = 1e-9
        weights_array = np.array(self.weights) + epsilon
        return -np.sum(weights_array * np.log2(weights_array))

    def resample(self):
        current_entropy = self.calculate_entropy()
        
        rospy.loginfo(f"Current Entropy: {current_entropy:.2f} / Entropy Threshold: {self.entropy_threshold:.2f} (Max: {self.max_entropy:.2f})")
        ...
        if current_entropy < self.entropy_threshold:
            rospy.logwarn("Kidnapping suspected (Entropy below threshold)! Performing adaptive resampling with random particle injection.")    
            
            normalized_entropy = current_entropy / self.max_entropy if self.max_entropy > 0 else 0
  
            if self.entropy_threshold > 0:
                
                lerp_factor = (self.entropy_threshold - current_entropy) / self.entropy_threshold
                injection_ratio = self.min_injection_ratio_entropy + \
                                  (self.max_injection_ratio_entropy - self.min_injection_ratio_entropy) * lerp_factor
                injection_ratio = np.clip(injection_ratio, self.min_injection_ratio_entropy, self.max_injection_ratio_entropy)
            else: 
                injection_ratio = self.max_injection_ratio_entropy
            ...
            if num_keep > 0:
                ...
                rospy.loginfo("All particles will be random due to very low entropy.")

            
            new_particles += self._create_uniform_particles()[:num_random]
            rospy.loginfo(f"Resampled {len(new_particles) - num_random} from weighted particles, injected {num_random} random. Injection Ratio: {injection_ratio:.2f}")
        else:
            ...
            for i in resample_idx:
                ...
                p_new.orientation.w = p_orig.orientation.w
                new_particles.append(p_new)
            rospy.loginfo("No kidnapping suspected. Standard resampling performed.")
    ...
    def estimate_pose(self):
        ...
        if not self.weights or not self.particles: 
            return est 
        ...
        if total_weight < 1e-9: 
            rospy.logwarn("Total weight is near zero, cannot estimate pose reliably. Returning default pose.")
            return est
        
        normalized_weights = [w / total_weight for w in self.weights]

        for p, w in zip(self.particles, normalized_weights):
            x += p.position.x * w
            y += p.position.y * w
            yaw = 2 * np.arctan2(p.orientation.z, p.orientation.w)
            z_sin += np.sin(yaw) * w
        ...
        if abs(w_cos) < 1e-9 and abs(z_sin) < 1e-9:
             avg_yaw = 0.0 
        else:
